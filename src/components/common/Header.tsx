...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 w-full z-50 bg-[var(--header-bg)] border-b-2 border-[var(--border-color)]">
            ...
            {isAuthenticated ? (
              <div className="relative group">
                ...
                <div className="absolute right-0 top-full mt-2 w-48 bg-[var(--bg-primary)] border-2 border-[var(--border-color)] shadow-[4px_4px_0_0_var(--shadow-color)] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                  ...
                  <Link
                    to={`/user/${user?.userId}`}
                    className="block w-full px-4 py-2 text-left hover:bg-[var(--bg-secondary)] transition-colors"
                  >
                    My Profile
                  </Link>
                  <button
